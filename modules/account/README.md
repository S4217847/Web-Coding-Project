...
different isolated development database. Obtain real Atlas values through a
...
password. Confirm `Connected to MongoDB database rmit_connect`, then open
...
In local development the page displays the demonstration reset link because no
mail provider is required by the assignment. In a public deployment, keep
`SHOW_DEMO_RESET_LINK=false` and deliver the raw link through a private channel.
The project does not claim to implement production email delivery.
